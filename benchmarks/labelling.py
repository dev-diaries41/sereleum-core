...
async def run(sereleum: PromptsManager, cluster_id: str, sample_size: int):
    result = sereleum.label_prompts(cluster_id, sample_size)
...
async def main():
    ...
    sereleum = PromptsManager(llm_client=llm, prompt_embedding_store=prompt_embedding_store, cluster_embedding_store=cluster_embedding_store)
    await run(sereleum, "fce4cfdc44b3ea3f", 10)
